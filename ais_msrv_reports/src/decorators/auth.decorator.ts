...
export function AuthDecorator(){
    ...
    return function(target: any, propertyName: string, descriptor: TypedPropertyDescriptor<Function>){
        ...
        descriptor.value = async function(...args) { // Переназначаем метод, который будет вызываться
            ...
            let permissions = await axios.post(`${process.env.AUTH_SERVICE_HOST}api/auth/bt_permissions`, {data: `${resource.uid},${args[0].sub}`}, {headers: {Authorization: `Bearer ${token}`}}) // забираем разрешения для данного пользователя и ресурса у сервиса авторизации
            // console.log(args)
            // console.log('qwdqw',permissions)
            if (permissions.data.length === 0){ // если нет никаких разрешений
                return new Message(403, 'Нет доступа на чтение')
            }
            else{
                ...
                if ((getFuncFlag) && (resource_meta[0] === 'post' || 'create' || 'update' || 'delete' || 'remove')){ // если есть разрешение на GET но обрабатываем POST
                    return new Message(403, 'Нет доступа на изменение')
                }
